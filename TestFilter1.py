...
##      信号の周波数領域におけるフィルタリング
yf      = fft.ifft(fft.fft(x, 2*M_x) * fft.fft(h_cw, 2*M_x))
yf      = np.real(yf)                           # FFT -> IFFT での虚数成分誤差を除去

omega_nx = 2 * np.arange(2*M_x) / (2*M_x)
omega_ny = 2 * np.arange(2*M_y) / (2*M_y)

##      プロット１
fig1, ax1 = plt.subplots(2, 1, constrained_layout=True)  # 2行1列のグラフウィンドウ作成
#   入力信号の振幅特性
ax1[0].plot(omega_nx, A_x)                      # 横軸を omega_n として A_bp をプロット
...
#   出力信号の振幅特性
ax1[1].plot(omega_ny, A_y)                      # 横軸を omega_n として P_bp をプロット
ax1[1].set_xlim(0, 1)                           # x軸の表示範囲を指定
ax1[1].set_title('Amplitude Characteristics for Output')
...
#   入力信号波形
ax2[0].plot(x)                                  # 入力信号波形
ax2[0].set_xlim(0, 2*N)
ax2[0].set_title('Waveform for Input')
ax2[0].set_xlabel('Angular Frequency (×π)');  ax2[0].set_ylabel('Value')
#   出力信号波形
ax2[1].plot(y)                                  # 出力信号波形(時間領域フィルタリング)
ax2[1].set_xlim(0, 2*N)
ax2[1].set_title('Waveform for Output')
...
#   入力信号波形
ax3[0].plot(x)                                  # 入力信号波形
ax3[0].set_xlim(0, 2*N)
ax3[0].set_title('Waveform for Input')
ax3[0].set_xlabel('Angular Frequency (×π)');  ax1[0].set_ylabel('Value')
#   出力信号波形
ax3[1].plot(yf)                                 # 出力信号波形(周波数領域フィルタリング)
ax3[1].set_xlim(0, 2*N)
ax3[1].set_title('Waveform for Output')
